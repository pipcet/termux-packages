termux_step_configure_cmake() {
	export TERMUX_PKG_USE_CCACHE=true
	...
	if [ "$TERMUX_ON_DEVICE_BUILD" = "false" ]; then
		...
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_FIND_USE_INSTALL_PREFIX=OFF")
	else
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_LINKER=$(command -v $LD) $LDFLAGS")
	fi

	if [ "$TERMUX_PKG_USE_CCACHE" = "true" ]; then
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_DISABLE_PRECOMPILE_HEADERS=TRUE")
	fi
